...
export default function App() {
  ...
  const fetchState = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        // If server is starting up, we might get 404 or 503
        console.warn(`Server not ready: ${res.status}`);
        return;
      }
      
      const contentType = res.headers.get('content-type');
      if (!contentType || !contentType.includes('application/json')) {
        // Probably getting index.html during startup
        return;
      }
      ...
      // Only log actual errors, not initial connection failures
      if (state) {
        console.error('Failed to fetch state:', err instanceof Error ? err.message : String(err));
      }
    }
  }, [state]);
      ...
      <header className="flex items-center justify-between hw-panel p-4">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-4 mr-4">
            ...
            <StatusIndicator label="JETSON" status={state.jetson.status} />
            <StatusIndicator label="GANTRY" status={state.gantry.status} />
            <StatusIndicator label="STIR" status={state.stirrer.status} />
            <StatusIndicator label="SCOOP" status={state.scoop.status} />
          </div>
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className={cn(
                      ...
                      LOGIC ENGINE {state.pythonBridge.toUpperCase()}
                    </div>
                  </div>
